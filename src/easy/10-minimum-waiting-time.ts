...
export function minimumWaitingTime(queries: readonly number[]): number {
  const sortedQueries = [...queries].sort((a, b) => a - b);
  let sum = 0;

  for (const [idx, duration] of sortedQueries.entries()) {
    const queriesLeft = sortedQueries.length - (idx + 1);
